...
export default function Dashboard() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const promises = [
        axios.get(API_URL, { params: { key: API_KEY, page_size: 1, dates: filterYear ? datesParam : undefined } }),
        ...
        axios.get(API_URL, { params: { key: API_KEY, page_size: 40, ordering: '-added', dates: datesParam } }), 
        axios.get(API_URL, { params: { key: API_KEY, page_size: 40, ordering: '-rating', dates: datesParam } }) 
      ];
      ...
      if (!filterYear) {
        ...
        genrePopularityData = allGenres.slice(0, 6).map(g => ({ genre: g.name, popularityScore: g.games_count }));
        ...
        // FILTERED MODE (Dynamic chart mapping from fetched year games)
        ...
        uniqueYearGames.forEach(g => {
          g.genres?.forEach(x => gCount[x.name] = (gCount[x.name] || 0) + 1);
        ...
        genrePopularityData = sortedG.slice(0, 6).map(x => ({ genre: x[0], popularityScore: x[1] }));
  ...
  useEffect(() => { fetchAllData(); }, [filterYear]);
  ...
  return (
    <Layout>
      ...
      <header className="dashboard-header fade-slide-up delay-1">
        ...
        <div className="search-input" style={{ display: 'flex', gap: '10px' }}>
          <select 
            value={filterYear} 
            onChange={(e) => setFilterYear(e.target.value)}
            className="header-filter-select"
            style={{ padding: '10px 15px', borderRadius: '8px', border: '1px solid #3b82f6', background: 'transparent', color: '#e5e7eb', outline: 'none' }}
          >
            <option value="" style={{ color: '#000' }}>Filter Tahun (Semua)</option>
